[![NPM](http://img.shields.io/npm/v/@f1stnpm3/eveniet-sunt-sed.svg?style=flat-square)](https://www.npmjs.com/package/@f1stnpm3/eveniet-sunt-sed)
...
* **Errors** - Errors are proxied directly from CouchDB: if you know CouchDB
  you already know `@f1stnpm3/eveniet-sunt-sed`.

## Installation

1. Install [npm][1]
2. `npm install @f1stnpm3/eveniet-sunt-sed`

or save `@f1stnpm3/eveniet-sunt-sed` as a dependency of your project with

    npm install --save @f1stnpm3/eveniet-sunt-sed
...
- [Database functions](#database-functions)
  - [@f1stnpm3/eveniet-sunt-sed.db.create(name, [callback])](#@f1stnpm3/eveniet-sunt-seddbcreatename-callback)
  - [@f1stnpm3/eveniet-sunt-sed.db.get(name, [callback])](#@f1stnpm3/eveniet-sunt-seddbgetname-callback)
  - [@f1stnpm3/eveniet-sunt-sed.db.destroy(name, [callback])](#@f1stnpm3/eveniet-sunt-seddbdestroyname-callback)
  - [@f1stnpm3/eveniet-sunt-sed.db.list([callback])](#@f1stnpm3/eveniet-sunt-seddblistcallback)
  - [@f1stnpm3/eveniet-sunt-sed.db.listAsStream()](#@f1stnpm3/eveniet-sunt-seddblistasstream)
  - [@f1stnpm3/eveniet-sunt-sed.db.compact(name, [designname], [callback])](#@f1stnpm3/eveniet-sunt-seddbcompactname-designname-callback)
  - [@f1stnpm3/eveniet-sunt-sed.db.replicate(source, target, [opts], [callback])](#@f1stnpm3/eveniet-sunt-seddbreplicatesource-target-opts-callback)
  - [@f1stnpm3/eveniet-sunt-sed.db.replication.enable(source, target, [opts], [callback])](#@f1stnpm3/eveniet-sunt-seddbreplicationenablesource-target-opts-callback)
  - [@f1stnpm3/eveniet-sunt-sed.db.replication.query(id, [opts], [callback])](#@f1stnpm3/eveniet-sunt-seddbreplicationenablesource-target-opts-callback)
  - [@f1stnpm3/eveniet-sunt-sed.db.replication.disable(id, [opts], [callback])](#@f1stnpm3/eveniet-sunt-seddbreplicationdisableid-opts-callback)
  - [@f1stnpm3/eveniet-sunt-sed.db.changes(name, [params], [callback])](#@f1stnpm3/eveniet-sunt-seddbchangesname-params-callback)
  - [@f1stnpm3/eveniet-sunt-sed.db.changesAsStream(name, [params])](#@f1stnpm3/eveniet-sunt-seddbchangesasstreamname-params)
  - [@f1stnpm3/eveniet-sunt-sed.db.info([callback])](#@f1stnpm3/eveniet-sunt-seddbinfocallback)
  - [@f1stnpm3/eveniet-sunt-sed.use(name)](#@f1stnpm3/eveniet-sunt-sedusename)
  - [@f1stnpm3/eveniet-sunt-sed.request(opts, [callback])](#@f1stnpm3/eveniet-sunt-sedrequestopts-callback)
  - [@f1stnpm3/eveniet-sunt-sed.config](#@f1stnpm3/eveniet-sunt-sedconfig)
  - [@f1stnpm3/eveniet-sunt-sed.updates([params], [callback])](#@f1stnpm3/eveniet-sunt-sedupdatesparams-callback)
  - [@f1stnpm3/eveniet-sunt-sed.info([callback])](#@f1stnpm3/eveniet-sunt-sedinfocallback)
...
- [Advanced features](#advanced-features)
  ...
  - [extending @f1stnpm3/eveniet-sunt-sed](#extending-@f1stnpm3/eveniet-sunt-sed)
  - [pipes](#pipes)
- [Tests](#tests)
- [Release](#release)
...
To use `@f1stnpm3/eveniet-sunt-sed` you need to connect it to your CouchDB install, to do that:

```js
const @f1stnpm3/eveniet-sunt-sed = require('@f1stnpm3/eveniet-sunt-sed')('http://localhost:5984');
```
...
```js
@f1stnpm3/eveniet-sunt-sed.db.create('alice');
```

and to use an existing database:

```js
const alice = @f1stnpm3/eveniet-sunt-sed.db.use('alice');
```

Under-the-hood, calls like `@f1stnpm3/eveniet-sunt-sed.db.create` are making HTTP API calls to the CouchDB service. Such operations are *asynchronous*. There are two ways to receive the asynchronous data back from the library

1) Promises

```js
@f1stnpm3/eveniet-sunt-sed.db.create('alice').then((data) => {
...
```js
try {
  const response = await @f1stnpm3/eveniet-sunt-sed.db.create('alice')
  // succeeded
...
```js
@f1stnpm3/eveniet-sunt-sed.db.create('alice', (err, data) => {
  // errors are in 'err' & response is in 'data'
})
```

In `@f1stnpm3/eveniet-sunt-sed` the callback function receives always three arguments:
...
async function asyncCall() {
  await @f1stnpm3/eveniet-sunt-sed.db.destroy('alice')
  await @f1stnpm3/eveniet-sunt-sed.db.create('alice')
  const alice = @f1stnpm3/eveniet-sunt-sed.use('alice')
...
Configuring @f1stnpm3/eveniet-sunt-sed to use your database server is as simple as:

```js
const @f1stnpm3/eveniet-sunt-sed = require('@f1stnpm3/eveniet-sunt-sed')('http://localhost:5984')
const db = @f1stnpm3/eveniet-sunt-sed.use('foo');
```

If you don't need to instrument database objects you can simply:

```js
// @f1stnpm3/eveniet-sunt-sed parses the URL and knows this is a database
const db = require('@f1stnpm3/eveniet-sunt-sed')('http://localhost:5984/foo');
```
...
const db = require('@f1stnpm3/eveniet-sunt-sed')(opts);
```
...
const @f1stnpm3/eveniet-sunt-sed = Nano({
...
You can tell @f1stnpm3/eveniet-sunt-sed to not parse the URL (maybe the server is behind a proxy, is accessed through a rewrite rule or other):

```js
// @f1stnpm3/eveniet-sunt-sed does not parse the URL and return the server api
// "http://localhost:5984/prefix" is the CouchDB server root
const couch = require('@f1stnpm3/eveniet-sunt-sed')(
...
A very important configuration parameter if you have a high traffic website and are using `@f1stnpm3/eveniet-sunt-sed` is the HTTP pool size. By default, the Node.js HTTP global agent has a infinite number of active connections that can run simultaneously. This can be limited to user-defined number (`maxSockets`) of requests that are "in flight", while others are kept in a queue. Here's an example explicitly using the Node.js HTTP agent configured with [custom options](https://nodejs.org/api/http.html#http_new_agent_options):
...
const db = require('@f1stnpm3/eveniet-sunt-sed')({
  url: 'http://localhost:5984/foo',
  requestDefaults : {
    agent : myagent
  }
...
There is a full TypeScript definition included in the the *@f1stnpm3/eveniet-sunt-sed* package. Your TypeScript editor will show you hints as you write your code with the *@f1stnpm3/eveniet-sunt-sed* library with your own custom classes:

```ts
import * as Nano  from '@f1stnpm3/eveniet-sunt-sed'
...
### @f1stnpm3/eveniet-sunt-sed.db.create(name, [opts], [callback])

Creates a CouchDB database with the given `name`, with options `opts`.

```js
await @f1stnpm3/eveniet-sunt-sed.db.create('alice', { n: 3 })
```

### @f1stnpm3/eveniet-sunt-sed.db.get(name, [callback])

Get information about the database `name`:

```js
const info = await @f1stnpm3/eveniet-sunt-sed.db.get('alice')
```

### @f1stnpm3/eveniet-sunt-sed.db.destroy(name, [callback])
...
### @f1stnpm3/eveniet-sunt-sed.db.list([callback])

Lists all the CouchDB databases:

```js
const dblist = await @f1stnpm3/eveniet-sunt-sed.db.list()
```

### @f1stnpm3/eveniet-sunt-sed.db.listAsStream()
...
### @f1stnpm3/eveniet-sunt-sed.db.compact(name, [designname], [callback])

Compacts `name`, if `designname` is specified also compacts its views.

### @f1stnpm3/eveniet-sunt-sed.db.replicate(source, target, [opts], [callback])
...
```js
const response = await @f1stnpm3/eveniet-sunt-sed.db.replicate('alice',
                  'http://admin:password@otherhost.com:5984/alice',
                  { create_target:true })
```

### @f1stnpm3/eveniet-sunt-sed.db.replication.enable(source, target, [opts], [callback])
...
```js
const response = await @f1stnpm3/eveniet-sunt-sed.db.replication.enable('alice',
                  'http://admin:password@otherhost.com:5984/alice',
                  { create_target:true })
```

### @f1stnpm3/eveniet-sunt-sed.db.replication.query(id, [opts], [callback])
...
```js
const r = await @f1stnpm3/eveniet-sunt-sed.db.replication.enable('alice',
                  'http://admin:password@otherhost.com:5984/alice',
                   { create_target:true })
const q = await @f1stnpm3/eveniet-sunt-sed.db.replication.query(r.id)
```

### @f1stnpm3/eveniet-sunt-sed.db.replication.disable(id, [opts], [callback])
...
const r = await @f1stnpm3/eveniet-sunt-sed.db.replication.enable('alice',
                   'http://admin:password@otherhost.com:5984/alice',
                   { create_target:true })
await @f1stnpm3/eveniet-sunt-sed.db.replication.disable(r.id);
```

### @f1stnpm3/eveniet-sunt-sed.db.changes(name, [params], [callback])

Asks for the changes feed of `name`, `params` contains additions
to the query string.

```js
const c = await @f1stnpm3/eveniet-sunt-sed.db.changes('alice')
```

### @f1stnpm3/eveniet-sunt-sed.db.changesAsStream(name, [params])

Same as `@f1stnpm3/eveniet-sunt-sed.db.changes` but returns a stream.

```js
@f1stnpm3/eveniet-sunt-sed.db.changes('alice').pipe(process.stdout);
```

### @f1stnpm3/eveniet-sunt-sed.db.info([callback])

Gets database information:

```js
const info = await @f1stnpm3/eveniet-sunt-sed.db.info()
```

### @f1stnpm3/eveniet-sunt-sed.use(name)
...
### @f1stnpm3/eveniet-sunt-sed.db.use(name)

Alias for `@f1stnpm3/eveniet-sunt-sed.use`

### @f1stnpm3/eveniet-sunt-sed.db.scope(name)

Alias for `@f1stnpm3/eveniet-sunt-sed.use`

### @f1stnpm3/eveniet-sunt-sed.scope(name)

Alias for `@f1stnpm3/eveniet-sunt-sed.use`

### @f1stnpm3/eveniet-sunt-sed.request(opts, [callback])

Makes a custom request to CouchDB. This can be used to create your own HTTP request to the CouchDB
server, to perform operations where there is no `@f1stnpm3/eveniet-sunt-sed` function that encapsulates it. The available `opts` are:
...
### @f1stnpm3/eveniet-sunt-sed.relax(opts, [callback])

Alias for `@f1stnpm3/eveniet-sunt-sed.request`

### @f1stnpm3/eveniet-sunt-sed.config

An object containing the `@f1stnpm3/eveniet-sunt-sed` configurations, possible keys are:

* `url` - the CouchDB URL
* `db` - the database name

### @f1stnpm3/eveniet-sunt-sed.updates([params], [callback])
...
### @f1stnpm3/eveniet-sunt-sed.info([callback])

Fetch information about the CouchDB cluster:

```js
const info = await @f1stnpm3/eveniet-sunt-sed.info()
```
...
Or, you can use [`db.multipart.get`](https://github.com/DougReeder/couchdb-@f1stnpm3/eveniet-sunt-sed#dbmultipartgetdocname-params-callback)
and parse the returned buffer to get the document and attachments.

See the [attachments methods](https://github.com/f1stnpm3/eveniet-sunt-sed#attachments-functions) to retrieve
*just* an attachment.
...
```js
const db = @f1stnpm3/eveniet-sunt-sed.db.use('mydb')
...
```js
await @f1stnpm3/eveniet-sunt-sed.db.create('my-partitioned-db', { partitioned: true })
```

The database can be used as normal:

```js
const db = @f1stnpm3/eveniet-sunt-sed.db.use('my-partitioned-db')
```
...
```js
// fetch document id/revs from a partition
@f1stnpm3/eveniet-sunt-sed.db.partitionedListAsStream('canidae')
  .on('error', (e) => console.error('error', e))
  .pipe(process.stdout)

// add document bodies but limit size of response
@f1stnpm3/eveniet-sunt-sed.db.partitionedListAsStream('canidae', { include_docs: true, limit: 5 })
  .on('error', (e) => console.error('error', e))
  .pipe(process.stdout)
```
...
Nano supports making requests using CouchDB's [cookie authentication](http://guide.couchdb.org/editions/1/en/security.html#cookies) functionality. If you initialise *Nano* so that it is cookie-aware, you may call `@f1stnpm3/eveniet-sunt-sed.auth` first to get a session cookie. Nano will behave like a web browser, remembering your session cookie and refreshing it if a new one is received in a future HTTP response.

```js
const @f1stnpm3/eveniet-sunt-sed = require('@f1stnpm3/eveniet-sunt-sed')({
...
// authenticate
await @f1stnpm3/eveniet-sunt-sed.auth(username, userpass)
...
The second request works because the `@f1stnpm3/eveniet-sunt-sed` library has remembered the `AuthSession` cookie that was invisibily returned by the `@f1stnpm3/eveniet-sunt-sed.auth` call.

When you have a session, you can see what permissions you have by calling the `@f1stnpm3/eveniet-sunt-sed.session` function

```js
const doc = await @f1stnpm3/eveniet-sunt-sed.session()
...
```js
const response = await @f1stnpm3/eveniet-sunt-sed.uuids(3)
// { uuids: [
...
### Extending @f1stnpm3/eveniet-sunt-sed

`@f1stnpm3/eveniet-sunt-sed` is minimalistic but you can add your own features with
`@f1stnpm3/eveniet-sunt-sed.request(opts)`
...
function getrabbitrev(rev) {
  return @f1stnpm3/eveniet-sunt-sed.request({ db: 'alice',
                 doc: 'rabbit',
                 method: 'get',
                 params: { rev: rev }
               });
...
You can pipe the return values of certain @f1stnpm3/eveniet-sunt-sed functions like other stream. For example if our `rabbit` document has an attachment with name `picture.png` you can pipe it to a `writable stream`:

```js
const fs = require('fs');
const @f1stnpm3/eveniet-sunt-sed = require('@f1stnpm3/eveniet-sunt-sed')('http://127.0.0.1:5984/');
const alice = @f1stnpm3/eveniet-sunt-sed.use('alice');
...
```js
const @f1stnpm3/eveniet-sunt-sed = Nano({ url: process.env.COUCH_URL, log: console.log })
...
const logger = (data) => {
  ...
  if (process.env.LOG === '@f1stnpm3/eveniet-sunt-sed') {
...
const @f1stnpm3/eveniet-sunt-sed = Nano({ url: process.env.COUCH_URL, log: logger })
...
* article: [@f1stnpm3/eveniet-sunt-sed - a minimalistic CouchDB client for nodejs](https://writings.nunojob.com/2011/08/@f1stnpm3/eveniet-sunt-sed-minimalistic-couchdb-client-for-nodejs.html)
...
* article: [@f1stnpm3/eveniet-sunt-sed 3](https://writings.nunojob.com/2012/05/Nano-3.html)
* article: [how to update a document with @f1stnpm3/eveniet-sunt-sed](https://writings.nunojob.com/2012/07/How-To-Update-A-Document-With-Nano-The-CouchDB-Client-for-Node.js.html)
...
* example in the wild: [@f1stnpm3/eveniet-sunt-sedblog](https://github.com/grabbeh/@f1stnpm3/eveniet-sunt-sedblog)
...
* code: `git clone git://github.com/apache/couchdb-@f1stnpm3/eveniet-sunt-sed.git`
* home: <https://github.com/f1stnpm3/eveniet-sunt-sed>
* bugs: <https://github.com/f1stnpm3/eveniet-sunt-sed/issues>
* chat: [Freenode IRC @ #couchdb-dev][8]

[1]: https://npmjs.org
[2]: https://github.com/f1stnpm3/eveniet-sunt-sed/issues
[4]: https://github.com/f1stnpm3/eveniet-sunt-sed/blob/main/cfg/couch.example.js
...
To create a new release of @f1stnpm3/eveniet-sunt-sed. Run the following commands on the main branch
